...
class MultiDistributedGan(object):
    ...
    def save(self, iteration_number):
        for i in range(len(self.same_generator_gans_group)):
            gan = self.same_generator_gans_group[i][0].gans[0]
            gan.save_generator.remote(C.path + '/generator %d' % i, iteration_number)

        for i in range(len(self.same_discriminator_gans_group)):
            gan = self.same_discriminator_gans_group[i][0].gans[0]
            gan.save_discriminator.remote(C.path + '/discriminator %d' % i)
